...
def get_history():
    username = session.get("username")
    history_cursor = sessions_collection.find(
        {"username": username},
        {
            "_id": 1,
            "timestamp": 1,
    ...
    # Convert cursor to list and process each item
    history = []
    for item in history_cursor:
        ...
        processed_item = {
            ...
            "video_emotion": item.get("video_emotion", latest_emotion),
            "video_score": item.get("video_score", latest_score),
            "audio_emotion": item.get("audio_emotion", latest_audio_sentiment),
            "audio_score": item.get("audio_score", latest_audio_score),
            "overall_sentiment": item.get("overall_sentiment", 
                latest_emotion if latest_score >= latest_audio_score else latest_audio_sentiment),
            "description": item.get("description", get_description("neutral", 0)),
            "session_duration": item.get("session_duration", 0),
            "session_ended_manually": item.get("session_ended_manually", False),
...
def view_results():
    username = session.get("username")
    results_cursor = sessions_collection.find(
        {"username": username},
        {
            "_id": 1,
            "timestamp": 1,
    ...
    for result in results_cursor:
        overall_sentiment = result.get("overall_sentiment", 
                                    latest_emotion if latest_score >= latest_audio_score 
                                    else latest_audio_sentiment)
        description = result.get("description", 
                              get_description(overall_sentiment, 
                                           max(result.get("video_score", 0), 
                                               result.get("audio_score", 0))))
        
        processed = {
            "_id": str(result["_id"]),
            "timestamp": result.get("timestamp", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            "video_emotion": result.get("video_emotion", latest_emotion),
            "video_score": result.get("video_score", latest_score),
            "audio_emotion": result.get("audio_emotion", latest_audio_sentiment),
            "audio_score": result.get("audio_score", latest_audio_score),
            "overall_sentiment": overall_sentiment,
            "description": description,
...
def session_details(session_id):
    try:
        ...
        # Ensure all emotion fields have proper values
        session_data["video_emotion"] = session_data.get("video_emotion", latest_emotion)
        session_data["video_score"] = session_data.get("video_score", latest_score)
        session_data["audio_emotion"] = session_data.get("audio_emotion", latest_audio_sentiment)
        session_data["audio_score"] = session_data.get("audio_score", latest_audio_score)
        session_data["overall_sentiment"] = session_data.get("overall_sentiment", latest_emotion if latest_score >= latest_audio_score else latest_audio_sentiment)
        session_data["description"] = session_data.get("description", get_description("neutral", 0))
        session_data["timestamp"] = session_data.get("timestamp", "Unknown date")
        session_data["chat_history"] = session_data.get("chat_history", [])
        session_data["session_duration"] = session_data.get("session_duration", 0)
        session_data["session_ended_manually"] = session_data.get("session_ended_manually", False)
